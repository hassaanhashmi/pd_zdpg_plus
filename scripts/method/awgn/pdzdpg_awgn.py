...
class PD_ZDPG():
    r"""
    Primal-Dual Zeroth-Order Determinitic Policy Gradient via Parameter Space Exploration
    """
    ...
    def get_actions(self):
        vec_actions = np.empty(self.vec_H.shape)
    ...
    def update_theta(self, delta_f, delta_fi):
      ...
      for i, policy in enumerate(self.policies):
            with torch.no_grad():
                    for j, (name, param) in enumerate(policy.named_parameters()):
                        if param.requires_grad:
                                param.data += self.lr_th*update_step*self.U_r[i*len(self.U_r)//len(self.policies)+j]
                                dict(policy.named_parameters())[name].data.copy_(param.data)
